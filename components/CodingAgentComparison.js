  ...
  return <div className="agent-plot" ref={host}>
    <svg viewBox={`0 0 ${width} ${height}`} role="group" aria-label={`${adjusted ? 'Subscription estimate' : 'Original API cost'} versus Coding Agent Index`}>
      ...
      <g className="model-clusters" clipPath={`url(#plot-${id})`} aria-label="Model family groups">
        {clusters.map(c=><path key={c.key} data-cluster={c.key} data-members={c.members.length} d={c.path} fill={c.key==='gemini'?`url(#gemini-${id})`:c.key==='anthropic'?'#ee9b70':'#171717'} fillOpacity={c.key==='codex'?.13:.27} stroke={c.key==='gemini'?`url(#gemini-${id})`:c.key==='anthropic'?'#db7549':'#171717'} strokeOpacity=".3" strokeWidth="1.2" />)}
      </g>
      ...
      {points.map(p => {
        ...
        return <g key={r.short} data-model={r.short} data-offscale={p.offscale || undefined} tabIndex="0" role="button" className="agent-point" aria-label={`${r.benchmark_label || r.label}. Score ${r.score.toFixed(2)}. ${adjusted ? 'Subscription estimate' : 'API cost'} ${money(adjusted ? r.price : r.api)} per task.${adjusted ? ` Range ${money(r.lo)} to ${money(r.hi)}.` : ''}${p.offscale ? ' Beyond the displayed cost axis.' : ''}`}
          ...
          {active && !p.offscale && <circle cx={px} cy={py} r="10" fill="none" stroke={c} strokeWidth="1.5" />}
          {p.offscale ? <path className="offscale-arrow" d={`M ${right-22} ${py} H ${right} M ${right-7} ${py-6} L ${right} ${py} L ${right-7} ${py+6}`} fill="none" stroke={c} strokeWidth="2" /> : r.historical ? <rect x={px - 4} y={py - 4} width="8" height="8" fill="white" stroke={c} strokeWidth="1.7" /> : cursor ? <path d={`M${px},${py-6} l6,6 -6,6 -6,-6 Z`} fill={c} /> : <circle cx={px} cy={py} r="4.5" fill={r.group === 'Antigravity' ? `url(#gemini-${id})` : c} stroke={r.group==='Antigravity'?'#4285f4':c} strokeWidth="1.7" />}
