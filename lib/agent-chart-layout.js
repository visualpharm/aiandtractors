...
export function chartLayout(models, width, adjusted, options = {}) {
  ...
  const scoreValues = options.scoreTicks || [55,60,65,70];
  ...
  return {width,height,left,right,top,bottom,costMin,costMax,points,clusters,scoreTicks:scoreValues.map(v=>({v,y:y(v)})),costTicks:(adjusted?(options.subscriptionTicks || [.05,.1,.2,.3]):(options.apiTicks || [1,3,6,9,12])).map(v=>({v,x:x(v)}))};
}
