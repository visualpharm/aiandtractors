export const FRONTIER = { scoreMin:55, scoreMax:72, apiMin:1, apiMax:12, subscriptionMin:.03, subscriptionMax:1 };
export const LAB_COLORS = { Codex:'#171717', 'Claude / Opus':'#db7549', 'Claude / Fable':'#db7549', Antigravity:'#4285f4', Grok:'#747b88', Kimi:'#2563eb' };
...
export function chartLayout(models, width, adjusted) {
  ...
  const points = frontierRows(models).filter(r => !adjusted || r.price != null).map(r => ({
    ...
    lo:r.lo == null ? null : Math.max(left,x(r.lo)), hi:r.hi == null ? null : Math.min(right,x(r.hi)),hiClipped:r.hi>costMax,
    text:agentLabel(r).join(' · '),lines:agentLabel(r),
  }));
  ...
  const groups=[['astra',p=>/^Astra (medium|high|xhigh|max)$/.test(p.row.short)],['anthropic',p=>p.row.group.startsWith('Claude /')],['gemini',p=>p.row.group==='Antigravity']];
  ...
  return {width,height,left,right,top,bottom,costMin,costMax,points,clusters,scoreTicks:[55,60,65,70].map(v=>({v,y:y(v)})),costTicks:(adjusted?[.05,.25,.5,.75,1]:[1,3,6,9,12]).map(v=>({v,x:x(v)}))};
}
